...
import { createLiveAuthority, attachLiveRoutes } from './live-authority.mjs';
...
const live = createLiveAuthority();
...
createServer(async (req, res) => {
  try {
    ...
    if (pathname.startsWith('/api/live/')) {
      if (attachLiveRoutes(req, res, live, SECURITY_HEADERS)) return;
    }
  ...
  console.log(`  Shared live feed → /api/live/feed`);
});
