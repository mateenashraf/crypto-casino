...
const SecureWeb3 = (() => {
  ...
  function getCasinoBalance(addr) {
    if (!addr) return 0;
    const raw = getStorage(CONFIG.STORAGE_BALANCES)[addr.toLowerCase()];
    const n = parseFloat(raw);
    if (!Number.isFinite(n) || n < 0) return 0;
    // Hard ceiling guards absurd forged values if seal were somehow bypassed
  ...
  function setCasinoBalance(addr, amount) {
    ...
    b[addr.toLowerCase()] = n.toFixed(6);
    setStorage(CONFIG.STORAGE_BALANCES, b);
  }

  function creditCasinoBalance(addr, deltaEth) {
    const d = Number(deltaEth);
    if (!addr || !Number.isFinite(d) || d === 0) return getCasinoBalance(addr);
    const next = Math.max(0, getCasinoBalance(addr) + d);
    setCasinoBalance(addr, next);
    return next;
  }
  ...
  async function connect() {
    if (!window.ethereum) {
      throw new Error('No Web3 wallet found. Install a browser wallet (MetaMask, Rabby, Coinbase Wallet, etc.).');
    }
