...
const SecureRuntime = (() => {
  ...
  // Lock storage API if already present; listen for tamper signals
  try {
  ...
  window.addEventListener('nd-storage-tamper', () => {
    try { window.SecureStorage?.reverifySensitive?.(); } catch { /* */ }
  });

  return { isProduction, isDevHost, scrubSensitiveGlobals };
})();

Object.defineProperty(window, 'SecureRuntime', {
  value: SecureRuntime,
  writable: false,
  configurable: false,
});
